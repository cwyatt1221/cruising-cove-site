...
**Board chat (v1):** Per-sailing chat for signed-in members only (`CommunityChatMessages`, partition = sailing key). Messages capped at 500 characters with a soft ~4s rate limit per author. Optional `channel` query/body (`board` default, or `book-trade`) keeps book-trade meetup chat separate from the main board room. UI on `/community/sailing.html` polls about every 4s while the tab is visible. Out of scope for v1: DMs, reactions, image uploads, typing indicators, delete-own-message.
